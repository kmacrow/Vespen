...
function PCEmulator(uh) {
    var mem, cpu, disk_xhr_backends, disks, i, p;
    mem = new Mem(uh.mem_size);
    cpu = new CPU_X86(mem);
    this.cpu = cpu;
    this.init_ioports();
    ...
    disks = [ "hda", "hdb"];
    disk_xhr_backends = new Array();
    for (i = 0; i < disks.length; i++) {
    	p = uh[disks[i]];
    	disk_xhr_backends[i] = null;
    	if (p) {
    		disk_xhr_backends[i] = new XhrDiskBackend(p.url, p.block_size, p.nb_blocks);
    	}
    }
    this.ide0 = new IDE_if(this, 0x1f0, 0x3f6, this.pic.set_irq.bind(this.pic, 14), disk_xhr_backends);
...
PCEmulator.prototype.load_binary = function(Gg, ha, callback) { return this.cpu.load_binary(Gg, ha, callback); };
